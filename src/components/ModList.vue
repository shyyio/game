...
import {listMods, MOD_REGISTRY_REPO_URL} from "@/client/ModRegistryClient.js";
...
/**
 * @param {object} mod
 * @returns {string} the command an operator pastes on their server
 */
function installCommand(mod) {
  return `pipes-server mods add ${mod.name}`;
}
 ...
 * @returns {Promise<void>}
 */
async function copyInstall(mod) {
  await navigator.clipboard.writeText(installCommand(mod));
  ...
  <div class="mod-list">
    <v-card class="mod-list-card" elevation="8">
      ...
      <v-card-subtitle>
        ...
        install them with <code>mods add</code>.
      </v-card-subtitle>
      <v-card-text>
        <v-text-field
            v-model="search"
        ...
        >
          ...
          <div v-if="expanded === mod.name" class="mod-row-detail" @click.stop>
            ...
            <div class="mod-row-install">
              <code>{{ installCommand(mod) }}</code>
              <v-btn size="small" variant="text" @click="copyInstall(mod)">
                {{ copied === mod.name ? "Copied" : "Copy" }}
              </v-btn>
            </div>
            <table class="mod-row-versions">
              <tr v-for="version in versionsOf(mod)" :key="version.version">
                <td>{{ version.version }}</td>
                <td class="mod-row-meta">SDK {{ version.sdkVersion }}</td>
                <td class="mod-row-meta">{{ version.publishedAt }}</td>
                <td class="mod-row-meta"><a :href="version.url" target="_blank" rel="noreferrer">files</a></td>
              </tr>
            </table>
          </div>
        </div>
      ...
      <v-card-actions>
        ...
        <v-spacer/>
        <v-btn variant="text" :href="MOD_REGISTRY_REPO_URL" target="_blank" rel="noreferrer">Publish a mod</v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
