...
test("one claimed chunk centers on that chunk", () => {
    const {camera} = build([chunkKeyAt(0, 0)]);
...
test("the center is the centroid of every claim, not their bounding box", () => {
    ...
    const {camera} = build([
        chunkKeyAt(0, 0),
        chunkKeyAt(CHUNK_SIZE, 0),
        chunkKeyAt(2 * CHUNK_SIZE, 0),
        chunkKeyAt(0, CHUNK_SIZE),
    ]);
...
test("claims across the origin average out to it", () => {
    const {camera} = build([chunkKeyAt(-CHUNK_SIZE, -CHUNK_SIZE), chunkKeyAt(0, 0)]);
...
test("gliding home keeps the current zoom", () => {
    const {camera, client} = build([chunkKeyAt(0, 0)]);
    camera.glideHome();
