...
import {ANIMATION_CYCLE_FRAMES, advanceAnimationFrame, currentAnimationFrame} from "@/client/layers/animation.js";
...
function ahead(steps) {
    return (currentAnimationFrame() + steps) % ANIMATION_CYCLE_FRAMES;
}
...
describe("animation clock", () => {
    it("holds the frame until a whole animation frame has elapsed", () => {
        ...
        assert.equal(advanceAnimationFrame(FRAME_MS / 2), (half + 1) % ANIMATION_CYCLE_FRAMES);
    });

    it("advances at the same rate whatever the refresh rate", () => {
        // A second is 24 animation frames, wherever in the cycle it starts.
        const at60 = ahead(24);
        for (let i = 0; i < 60; i++) {
    ...
    it("steps the whole count over a long delta, keeping the phase", () => {
        const start = ahead(0);
        assert.equal(advanceAnimationFrame(FRAME_MS * 3), (start + 3) % ANIMATION_CYCLE_FRAMES);
        // The leftover part-frame carries, so a half-frame delta still steps.
        const carried = ahead(0);
        advanceAnimationFrame(FRAME_MS * 1.5);
        assert.equal(advanceAnimationFrame(FRAME_MS * 0.5), (carried + 2) % ANIMATION_CYCLE_FRAMES);
    });
});
