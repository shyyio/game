...
import {isToolbarVisible, counterListTop, isFriendsPanelRefreshDue, isMountedInPanelHost} from "@/client/Hud.js";
...
test("the toolbar is up in world view once the player holds a chunk", () => {
    assert.equal(isToolbarVisible(HAS_CLAIMS, ViewMode.WORLD), true);
});

test("the toolbar stays down with nothing to build on", () => {
    assert.equal(isToolbarVisible(NO_CLAIMS, ViewMode.WORLD), false);
});

test("the toolbar stays down while zoomed out, claims or not", () => {
    assert.equal(isToolbarVisible(HAS_CLAIMS, ViewMode.MAP), false);
    assert.equal(isToolbarVisible(HAS_CLAIMS, ViewMode.OVERWORLD), false);
});
...
test("the friend roster rebuilds once the throttle window has passed", () => {
    assert.equal(isFriendsPanelRefreshDue(NOW, NOW - FRIENDS_PANEL_REFRESH_THROTTLE_MS), true);
    assert.equal(isFriendsPanelRefreshDue(NOW, NOW - FRIENDS_PANEL_REFRESH_THROTTLE_MS + 1), false);
});

test("the first move of a session rebuilds the roster", () => {
    assert.equal(isFriendsPanelRefreshDue(NOW, 0), true);
});

test("a mod's band-less panel layer mounts into the panel host", () => {
    assert.equal(isMountedInPanelHost(new Container()), true);
});

test("a mod's tooltip keeps the band it brought, so it mounts on the stage", () => {
    const tooltip = new Container();
    tooltip.zIndex = HudLayer.TOOLTIP;
    assert.equal(isMountedInPanelHost(tooltip), false);
});
