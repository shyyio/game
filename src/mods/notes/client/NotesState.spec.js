...
import {ChunkUnsubscribeEvent, ClientCache, PLAYER_REF_NONE, chunkKey, tileKey} from "@spup/sdk/client";
...
test("a set event writes the note, an edit replaces it", () => {
    ...
    const note = state.mapGet("notes.byTile", tileKey(3, 4));
    ...
    assert.equal(state.mapGet("notes.byTile", tileKey(3, 4)).text, "second");
});

test("an edit refreshes the hovered note", () => {
    const state = stateWithNotes();
    state.onEvent(new NoteSetEvent(3, 4, 0, 0, 7, "first"));
    state.writer("notes").setHover(state.mapGet("notes.byTile", tileKey(3, 4)));
...
test("a delete drops the note and any target standing on it", () => {
    ...
    writer.setHover(state.mapGet("notes.byTile", tileKey(3, 4)));
    writer.openEditor(new NoteEditorTarget(3, 4, 0, 0, "first", NOTE_EDITOR_MODE_EDIT, 7));

    state.onEvent(new NoteDeleteEvent(3, 4));
    assert.equal(state.mapGet("notes.byTile", tileKey(3, 4)), undefined);
...
test("unsubscribing a chunk evicts its notes and targets", () => {
    ...
    state.onEvent(new ChunkUnsubscribeEvent(chunkKey(3, 4)));
    assert.equal(state.mapGet("notes.byTile", tileKey(3, 4)), undefined);
    assert.equal(state.mapGet("notes.byTile", tileKey(1000, 1000)).text, "far away");
    assert.equal(state.get("notes.hoverTarget"), null);
});
