...
test("a placed belt bakes as one run at its tile", async () => {
    ...
    const event = engine.overworldTiles.getSnapshotInRect(0, 0, 1, 1);
...
test("the bake survives a serialize/deserialize round-trip", async () => {
    ...
    const before = engine.overworldTiles.getSnapshotInRect(-2, -2, 4, 4);

    const restored = await makeGameEngine();
    restored.snapshots.deserialize(engine.snapshots.serialize());
    const after = restored.overworldTiles.getSnapshotInRect(-2, -2, 4, 4);
...
test("a rect outside the region throws", async () => {
    const engine = await makeGameEngine();
    assert.throws(() => engine.overworldTiles.getSnapshotInRect(-100, 0, 64, 1), RangeError);
});
