...
function drain(engine, laneRef, ticks) {
    ...
    for (let i = 0; i < ticks; i += 1) {
        engine.ports.setItem(engine.lanes.outputPortOf(laneRef), EMPTY);
        engine.tick();
        if (engine.ports.item(engine.lanes.outputPortOf(laneRef)) === CARGO) {
            delivered += 1;
        }
    }
    return delivered;
...
test("extending a lane downstream preserves an item resting in the output port", async () => {
    ...
    for (let i = 0; i < 10 && engine.ports.item(engine.lanes.outputPortOf(lane)) !== CARGO; i += 1) {
        engine.tick();
    }
    assert.equal(engine.ports.item(engine.lanes.outputPortOf(lane)), CARGO, "the item rests in the output port");
...
test("packed same-type items survive a split and each still pops", async () => {
    ...
    for (let i = 0; i < 12; i += 1) {
        engine.ports.setItem(engine.lanes.inputPortOf(fed), CARGO);
        engine.tick();
    }
    ...
    engine.ports.setItem(engine.lanes.inputPortOf(sub), EMPTY);
...
test("items on cells orphaned by a junction steal survive the rebuild", async () => {
    ...
    engine.ports.setItem(engine.lanes.inputPortOf(orphan), EMPTY);
...
test("an in-flight item survives deletion of a downstream cell and is still delivered", async () => {
    ...
    engine.ports.setItem(engine.lanes.inputPortOf(laneAt(engine, 0, 4)), CARGO);
    engine.tick();
    engine.tick();
    ...
    assert.equal(drain(engine, laneAt(engine, 0, 4), 12), 1, "and is still delivered to the shortened lane's output port");
});
...
test("filling a gap to merge two lanes keeps the source's in-flight item", async () => {
    ...
    engine.ports.setItem(engine.lanes.inputPortOf(laneAt(engine, 12, 6)), CARGO);
    engine.tick();
...
test("filling a gap to merge two lanes keeps an item resting in the sink's input port", async () => {
    ...
    placeLane(engine, 12, 6, Direction.UP);
    placeLane(engine, 12, 4, Direction.UP);
    engine.ports.setItem(engine.lanes.inputPortOf(laneAt(engine, 12, 4)), CARGO);
...
test("a tail merge re-ingests a resting output port item onto the connecting cell", async () => {
    ...
    const oldOutputPort = engine.lanes.outputPortOf(laneAt(engine, 0, 0));
    engine.ports.setItem(oldOutputPort, CARGO);

    placeLane(engine, 3, 0, Direction.RIGHT);

    assert.equal(engine.ports.item(oldOutputPort), EMPTY, "the old output port is cleared");
...
test("a junction steal leaves the boundary item on the orphan's output port, not the stealing lane", async () => {
    ...
    const orphanOut = engine.lanes.outputPortOf(orphan);
    ...
    assert.equal(engine.ports.item(orphanOut), CARGO, "the boundary item rests on the orphan's output port");
