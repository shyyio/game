...
test("subscribing answers once immediately, and again on the next push", async () => {
    ...
    game.dispatchMessage(
        new MetricsSubscribeMessage(METRICS_EVENT_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN, 10, 1000),
        alice,
    );
    await Promise.resolve();
    assert.equal(rollupEventsOf(alice).length, 1);

    game.metrics.record(METRICS_EVENT_TYPE_ITEM_PRODUCED, 1, 5, 1);
    await game.metrics.flush();
    // Bucket 0 (ticks 0-9) only completes once the clock reaches 10 (MetricsSubscriptions.push()).
    game.simEngine.clock = 10;
    game.pushMetricsUpdates();
    await Promise.resolve();
...
test("unsubscribe stops future pushes", async () => {
    ...
    await Promise.resolve();
    game.dispatchMessage(new MetricsUnsubscribeMessage(METRICS_EVENT_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN), alice);
    alice.events.length = 0;
...
test("two sessions sharing a GLOBAL subscription share one push, each their own for OWN", async () => {
    ...
    game.metrics.record(METRICS_EVENT_TYPE_ITEM_PRODUCED, 1, 42, 1);
    game.metrics.record(METRICS_EVENT_TYPE_ITEM_PRODUCED, 2, 99, 1);
    await game.metrics.flush();

    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_EVENT_TYPE_TRADE_EXECUTED, METRICS_QUERY_SCOPE_GLOBAL, 10, 1000), alice);
    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_EVENT_TYPE_TRADE_EXECUTED, METRICS_QUERY_SCOPE_GLOBAL, 10, 1000), bob);
    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_EVENT_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN, 10, 1000), alice);
    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_EVENT_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN, 10, 1000), bob);
    await Promise.resolve();
    alice.events.length = 0;
    bob.events.length = 0;
    ...
    const aliceGlobal = rollupEventsOf(alice).find(e => e.metricsType === METRICS_EVENT_TYPE_TRADE_EXECUTED);
    const bobGlobal = rollupEventsOf(bob).find(e => e.metricsType === METRICS_EVENT_TYPE_TRADE_EXECUTED);
    assert.deepEqual(aliceGlobal.sum, [300]);
    assert.deepEqual(bobGlobal.sum, [300]);

    const aliceOwn = rollupEventsOf(alice).find(e => e.metricsType === METRICS_EVENT_TYPE_ITEM_PRODUCED);
    const bobOwn = rollupEventsOf(bob).find(e => e.metricsType === METRICS_EVENT_TYPE_ITEM_PRODUCED);
...
test("disconnect clears subscriptions so a later push touches nothing for that session", async () => {
    ...
    assert.doesNotThrow(() => game.pushMetricsUpdates());
});
