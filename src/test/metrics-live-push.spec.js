...
test("subscribing answers once immediately, and again on the next push", async () => {
    ...
    game.dispatchMessage(
        new MetricsSubscribeMessage(METRICS_ENTRY_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN, 10, 1000),
        alice,
    );
    await Promise.resolve();
    assert.equal(rollupEventsOf(alice).length, 1);

    game.metrics.emitMetricsEntry(METRICS_ENTRY_TYPE_ITEM_PRODUCED, 1, 5, 1);
...
test("unsubscribe stops future pushes", async () => {
    ...
    await Promise.resolve();
    game.dispatchMessage(new MetricsUnsubscribeMessage(METRICS_ENTRY_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN), alice);
    alice.events.length = 0;
...
test("two sessions sharing a GLOBAL subscription share one push, each their own for OWN", async () => {
    ...
    game.metrics.emitMetricsEntry(METRICS_ENTRY_TYPE_ITEM_PRODUCED, 1, 42, 1);
    game.metrics.emitMetricsEntry(METRICS_ENTRY_TYPE_ITEM_PRODUCED, 2, 99, 1);
    await game.metrics.flush();

    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_ENTRY_TYPE_TRADE_EXECUTED, METRICS_QUERY_SCOPE_GLOBAL, 10, 1000), alice);
    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_ENTRY_TYPE_TRADE_EXECUTED, METRICS_QUERY_SCOPE_GLOBAL, 10, 1000), bob);
    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_ENTRY_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN, 10, 1000), alice);
    game.dispatchMessage(new MetricsSubscribeMessage(METRICS_ENTRY_TYPE_ITEM_PRODUCED, METRICS_QUERY_SCOPE_OWN, 10, 1000), bob);
    await Promise.resolve();
    alice.events.length = 0;
    bob.events.length = 0;

    // See the previous test: bucket 0 (where every entry above landed) only completes at tick 10.
    game.simEngine.clock = 10;
    game.metrics.push();
    await Promise.resolve();

    const aliceGlobal = rollupEventsOf(alice).find(e => e.metricsType === METRICS_ENTRY_TYPE_TRADE_EXECUTED);
    const bobGlobal = rollupEventsOf(bob).find(e => e.metricsType === METRICS_ENTRY_TYPE_TRADE_EXECUTED);
    assert.deepEqual(aliceGlobal.sum, [300]);
    assert.deepEqual(bobGlobal.sum, [300]);

    const aliceOwn = rollupEventsOf(alice).find(e => e.metricsType === METRICS_ENTRY_TYPE_ITEM_PRODUCED);
    const bobOwn = rollupEventsOf(bob).find(e => e.metricsType === METRICS_ENTRY_TYPE_ITEM_PRODUCED);
