...
test("the whole world round-trips through the engine serializer", async () => {
    ...
    const outputPort = restored.ports.at(5, 4, Direction.UP);
    assert.deepEqual(restored.render.portTile(outputPort), {x: 5, y: 4}, "output port re-registered at its own tile");
    for (const tile of [{x: 3, y: 7}, {x: 4, y: 7}]) {
        const port = restored.ports.at(tile.x, tile.y, Direction.UP);
        assert.deepEqual(restored.render.portTile(port), tile, "splitter output port re-registered at its own tile");
    }
    let produced = false;
    for (let i = 0; i < 8 && !produced; i += 1) {
        restored.tick();
        produced = restored.ports.item(outputPort) === ITEM_TYPE_WATER;
    }
...
test("a snapshot survives a JSON blob round-trip (the client save path)", async () => {
    ...
    assert.notEqual(restored.placed.eidByObjectRef(splitterId), undefined);
});
...
test("a snapshot round-trips through structured SQLite (the node save path)", async () => {
    ...
    assert.equal(restored.placed.eidsOf(ExtractorType.objectTypeId).length, 1);
    assert.equal(restored.placed.eidsOf(BlenderType.objectTypeId).length, 1);
});
