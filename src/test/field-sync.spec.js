...
function placeGate(game, player, x, y) {
    ...
    game.dispatchMessage(new CreateObjectMessage(GateDefinition.typeId, x, y, Direction.UP), player);
...
test("a marked row's synced fields batch per chunk at tick end, to the chunk's viewers only", async () => {
    ...
    game.dispatchMessage(new CreateObjectMessage(GateDefinition.typeId, 300, 300, Direction.UP), far);
...
test("chunk sync carries every row off its defaults, after the objects themselves", async () => {
    ...
    game.dispatchMessage(new CreateObjectMessage(GateDefinition.typeId, 6, 5, Direction.UP), player);
