...
function connected(engine, ax, ay, bx, by) {
    const a = engine.belts.pathAt(ax, ay);
    const b = engine.belts.pathAt(bx, by);
    return a !== null && b !== null && a.id === b.id;
}

function itemCells(engine) {
    return engine.belts.paths.flatMap(path => path.items).filter(run => run.type !== 0).reduce((sum, run) => sum + run.length, 0);
}
...
test("adjacent ramps connect into one tunnel path", async () => {
    ...
    assert.equal(engine.belts._allBelts().length, 2, "no undergrounds between adjacent ramps");
});
...
test("ramps connect at the maximum tunnel length", async () => {
    ...
    assert.equal(engine.belts._allBelts().length, MAX_UNDERGROUND_LENGTH + 2);
});
...
test("ramps beyond the maximum tunnel length do not connect", async () => {
    ...
    assert.equal(engine.belts.paths.length, 2);
});
...
test("a reversed pair (ramp-up first, then ramp-down) connects", async () => {
    ...
    assert.equal(engine.belts.paths.length, 1);
});
...
test("deleting the up ramp collapses the tunnel, leaving the down ramp", async () => {
    ...
    assert.equal(engine.belts._allBelts().length, 1, "the ramp-up and its undergrounds are gone");
    assert.equal(engine.belts.paths.length, 1);
    assert.equal(engine.belts.paths[0].length, 1, "the surviving ramp-down is a standalone belt");
});
...
test("deleting the down ramp collapses the tunnel, leaving the up ramp", async () => {
    ...
    assert.equal(engine.belts._allBelts().length, 1, "the ramp-down and its undergrounds are gone");
    assert.equal(engine.belts.paths.length, 1);
    assert.equal(engine.belts.paths[0].length, 1);
});
