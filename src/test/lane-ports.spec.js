...
test("a lane's in and output ports are the shared edge ports a neighbor would adopt", async () => {
    ...
    assert.equal(engine.lanes.inputPortOf(lane), engine.ports.at(0, 2, Direction.UP), "input port = the head tile's back edge");
    assert.equal(engine.lanes.outputPortOf(lane), engine.ports.at(0, -1, Direction.UP), "output port = the edge past the tail");
});
...
test("a machine placed at a lane's output adopts its output port and receives items", async () => {
    ...
    assert.equal(engine.ports.at(5, 5, Direction.UP), outputPort, "the machine's input is the lane's output port");

    engine.ports.setItem(engine.lanes.inputPortOf(lane), ITEM_TYPE_TEST_MACHINE_INPUT);
    let taken = false;
    for (let i = 0; i < 12 && !taken; i += 1) {
        engine.tick();
        taken = engine.lanes.itemCountOf(lane) === 0 && engine.ports.item(outputPort) !== ITEM_TYPE_TEST_MACHINE_INPUT;
    }
...
test("a lane's ports survive the unreferenced-port sweep", async () => {
    ...
    assert.equal(engine.lanes.inputPortOf(lane), inputPort, "the input port survived");
    assert.equal(engine.lanes.outputPortOf(lane), outputPort, "the output port survived");
    assert.equal(engine.ports.at(0, 2, Direction.UP), inputPort, "and is still the tile edge's shared port");
});
