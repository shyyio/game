...
test("a volcano feeds a primary extractor (sulfur) and a deep extractor (brine) on its ring", async () => {
    ...
    assert.equal(engine.space.getUserDataAt(5, 4, LAYER_RESOURCE), 900, "ring tile is covered by volcano");
    assert.equal(engine.space.getUserDataAt(5, 5, LAYER_RESOURCE), null, "the 2x2 body is not an extraction tile");
