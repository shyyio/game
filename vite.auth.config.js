...
// artifact is build/authserver plus an install of just the external deps; the unbundled
// `npm run serve:auth` dev path (src/nodeservice/loader.js hooks) is unaffected.
export default defineConfig(({mode}) => ({
    // The public dir (favicon) belongs to the browser build only.
    publicDir: false,
    build: {
        ssr: "src/authserver/main.js",
        outDir: "build/authserver",
        target: "node20",
        rollupOptions: {
